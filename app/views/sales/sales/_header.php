...
$toolbar_items = (!$model->isNewRecord && $model->status == Sales::STATUS_DRAFT && $this->context->action->id == 'update' ) ?
...
$toolbar_items = (!$model->isNewRecord && $model->status == Sales::STATUS_DRAFT && $this->context->action->id == 'view') ?
...
$toolbar_items = (!$model->isNewRecord && $model->status > Sales::STATUS_DRAFT && $model->status < Sales::STATUS_PROCESS ) ?
        array_merge($toolbar_items, [['label' => 'Deliver', 'url' => ['/inventory/movement/create', 'type' => 200, 'id' => $model->id], 'linkOptions' => ['id' => 'deliver', 'class' => 'btn btn-success btn-sm']]]) :
        array_merge($toolbar_items, [['label' => 'Deliver', 'linkOptions' => ['id' => 'deliver', 'class' => 'btn btn-primary btn-sm disabled']]]);
$toolbar_items = (!$model->isNewRecord && $model->status == Sales::STATUS_PROCESS) ?
        array_merge($toolbar_items, [['label' => 'Invoice', 'url' => ['/accounting/invoice/create', 'type' => 200, 'id' => $model->id], 'linkOptions' => ['id' => 'invoice', 'class' => 'btn btn-success btn-sm']]]) :
